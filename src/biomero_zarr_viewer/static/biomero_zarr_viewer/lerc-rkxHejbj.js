...
import { g as DA, B as nA } from "./main-sa1dqITM.js";
const fA = {
  AddCompression: 1
}, gA = {
  None: 0,
      ...
      var J = (function() {
        ...
        var u = {
            ...
            for (r = D; r < B; r++)
              if (s = r - (r < g ? 0 : g), e = o[s].first, e > 0)
                if (k = [e, s], e <= n)
                  ...
                  for (M = o[s].second, d = l, N = e - 1; N >= 0; N--)
                    q = M >>> N & 1, q ? (d.right || (d.right = new m()), d = d.right) : (d.left || (d.left = new m()), d = d.left), N === 0 && !d.val && (d.val = k[1]);
            return {
              decodeLut: y,
              numBitsLUTQick: n,
              numBitsLUT: w,
          ...
          readHuffman: function(E, A, I, a) {
            ...
            i > 0 && (e++, i = 0);
            var q = f[e], d = A.encodeMode === 1, O = new I(C * g), v = O, Y;
            if (g < 2 || d) {
              for (Y = 0; Y < g; Y++)
                if (g > 1 && (v = new I(O.buffer, C * Y, C), N = 0), A.headerInfo.numValidPixel === B * D)
                  for (M = 0, y = 0; y < D; y++)
                    for (k = 0; k < B; k++, M++) {
                      ...
                      i >= 32 && (i -= 32, e++, q = f[e]), G = U - S, d ? (k > 0 ? G += N : y > 0 ? G += v[M - B] : G += N, G &= 255, v[M] = G, N = G) : v[M] = G;
                    }
                else
                  for (M = 0, y = 0; y < D; y++)
                    for (k = 0; k < B; k++, M++)
                      if (R[M]) {
                        ...
                        i >= 32 && (i -= 32, e++, q = f[e]), G = U - S, d ? (k > 0 && R[M - 1] ? G += N : y > 0 && R[M - B] ? G += v[M - B] : G += N, G &= 255, v[M] = G, N = G) : v[M] = G;
                      }
            } else
              for (M = 0, y = 0; y < D; y++)
                for (k = 0; k < B; k++)
          ...
          readTiles: function(E, A, I, a) {
            ...
            var e = 0, i = 0, t = 0, F = 0, S = 0, h = 0, U = 0, G = 0, R = 0, w = 0, n = 0, l = 0, y = 0, k = 0, M = 0, c = 0, N, q, d, O, v, Y, P = new I(C * C), aA = D % C || C, rA = g % C || C, AA, b, $ = Q.numDims, W, K = A.pixels.resultMask, H = A.pixels.resultPixels, oA = Q.fileVersion, EA = oA >= 5 ? 14 : 15, p, IA = Q.zMax, V;
            for (t = 0; t < f; t++)
              for (S = t !== f - 1 ? C : aA, F = 0; F < s; F++)
                for (h = F !== s - 1 ? C : rA, n = t * g * C + F * C, l = g - h, W = 0; W < $; W++) {
                  ...
                  if (Y === 2) {
                    ...
                    if (p)
                      throw "integrity issue";
                    if (A.counter.uncompressed++, A.ptr += c, y = S * h * r, k = E.byteLength - A.ptr, y = y < k ? y : k, d = new ArrayBuffer(y % r === 0 ? y : y + r - y % r), O = new Uint8Array(d), O.set(new Uint8Array(E, A.ptr, y)), v = new I(d), M = 0, K)
                      for (e = 0; e < S; e++) {
                        for (i = 0; i < h; i++)
                          K[n] && (H[n] = v[M++]), n++;
                        n += l;
        ...
        var u = new ArrayBuffer(4), L = new Uint8Array(u), m = new Uint32Array(u);
        return m[0] = 1, L[0] === 1;
      })(), X = {
        /************wrapper**********************************************/
        /**
        ...
        **/
        decode: function(u, L) {
          ...
          L = L || {};
          var m = L.inputOffset || 0, z = new Uint8Array(u, m, 10), E = String.fromCharCode.apply(null, z), A, I;
          if (E.trim() === "CntZImage")
            A = J, I = 1;
          ...
          for (var a = 0, Q = u.byteLength - 10, g, D = [], B, C, o = {
            ...
            m = s.fileInfo.eofOffset, C = s.maskData, a === 0 && (g = s.encodedMaskData, o.width = s.width, o.height = s.height, o.dimCount = s.dimCount || 1, o.pixelType = s.pixelType || s.fileInfo.pixelType, o.mask = C), I > 1 && (C && D.push(C), s.fileInfo.mask && s.fileInfo.mask.numBytes > 0 && r++), a++, o.pixels.push(s.pixelData), o.statistics.push({
              minValue: s.minValue,
              maxValue: s.maxValue,
...
class GA {
  ...
  decode(J, T = 0) {
    ...
    const u = x.exports.malloc(T), L = x.exports.ZSTD_decompress(u, T, X, Z), m = CA.slice(u, u + L);
    return x.exports.free(X), x.exports.free(u), m;
  }
}
